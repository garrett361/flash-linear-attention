...
class GatedDeltaNetBlockCP(GradientCheckpointingLayer):
    """GatedDeltaNet Block with Context Parallelism support"""
    ...
    def forward(
        ...
        **kwargs: Unpack[Dict]
        ...
        # Pass through context parallel parameters
        hidden_states, attentions, past_key_values = self.attn(
            ...
            **kwargs  # This includes cp_rank, cp_size, cp_group
        )
...
class GatedDeltaNetModelCP(GatedDeltaNetPreTrainedModel):
    """GatedDeltaNet Model with Context Parallelism support"""
    ...
    def __init__(self, config: GatedDeltaNetConfig):
        ...
        self.layers = nn.ModuleList([
            GatedDeltaNetBlockCP(config, layer_idx) 
    ...
    def forward(
        ...
        **kwargs: Unpack[Dict]
        ...
        # Pass context parallel parameters through all layers
        for layer in self.layers:
            ...
            hidden_states, attentions, past_key_values = layer(
                ...
                use_cache=use_cache,
                output_attentions=output_attentions,
                **kwargs  # This propagates cp_rank, cp_size, cp_group
            )
...
class GatedDeltaNetForCausalLMCP(GatedDeltaNetPreTrainedModel, FLAGenerationMixin):
    """GatedDeltaNet for Causal Language Modeling with Context Parallelism support"""

    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = GatedDeltaNetModelCP(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        **kwargs: Unpack[Dict]
        ...
        # Forward pass with context parallel parameters propagated via kwargs
        outputs = self.model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            ...
            **kwargs  # Propagates cp_rank, cp_size, cp_group
        )
